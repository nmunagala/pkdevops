class nexus (
    ...
    $nexus_folder = "nexus-3.0.1-01",
    $nexus_home = "/opt/nexus-3.0.1" ) {
     ...
     file {"nexus_archive":
       ...
       path   => "${files_path}/nexus-3.0.1-01-unix.tar.gz",
     }
     ...
     exec { "Extract Nexus":
       command => "tar -xvf ${files_path}/nexus-3.0.1-01-unix.tar.gz",
       cwd     => "/tmp",
       creates => "${nexus_home}",
     ...
     exec { "Run Installer":
       command => "sudo /bin/bash -c '/opt/nexus-3.0.1/bin/nexus run &'",
     }
}
